WITH hourly_raw AS (
    SELECT 
        airport_code,
        station_id,
        JSON_ARRAY_ELEMENTS(extracted_data -> 'data') AS json_data
    FROM {{source('weather_data', 'weather_hourly_raw')}}
),

hourly_flattened AS (
    SELECT 
        airport_code,
        station_id,
        (json_data ->> 'time')::timestamp AS timestamp,
        (json_data ->> 'temp')::NUMERIC AS temp_c,
        (json_data ->> 'dwpt')::NUMERIC AS dew_point_c,
        (json_data ->> 'rhum')::NUMERIC AS humidity_pct,
        (json_data ->> 'prcp')::NUMERIC AS precipitation_mm,
        (json_data ->> 'snow')::NUMERIC::INTEGER AS snow_mm,
        (json_data ->> 'wdir')::NUMERIC::INTEGER AS wind_direction_deg,
        (json_data ->> 'wspd')::NUMERIC AS wind_speed_ms,
        (json_data ->> 'wpgt')::NUMERIC AS wind_gust_ms,
        (json_data ->> 'pres')::NUMERIC AS pressure_hpa,
        (json_data ->> 'tsun')::NUMERIC::INTEGER AS sunshine_minutes
    FROM hourly_raw
)

SELECT * 
FROM hourly_flattened